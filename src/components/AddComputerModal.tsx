...
export default function AddComputerModal({ onClose, onSuccess }: AddComputerModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await computerService.create(name.trim(), location.trim() || undefined);
      onSuccess();
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? 'Adding...' : 'Add Computer'}
            </button>
          </div>
        </form>
      </div>
